...
export const generateStaticParams = async () => {
  const projects = await queryBlogs({
    queryLength: 50,
    orderByRecency: true,
...
export const generateMetadata = async ({
  ...
  const fetched = await queryBlogs({
    slug: escapeQueryString(slug),
    queryLength: 1,
    lookingFor: ['title', 'description'],
  });
...
const BlogPage = async ({ params: { slug } }: { params: { slug: string } }) => {
        ...
        <div className="mt-8 flex w-4/5 flex-col items-center max-sm:w-[97.5%]">
          <h1 className="mx-auto w-fit text-5xl font-bold sm:w-1/2">
            ...
            <span className="mt-2 block text-center text-5xl leading-8 tracking-tight sm:text-6xl">
              Looks like this blog doesn&apos;t exist!
            </span>
          </h1>
  ...
  const newUpdated =
    data.timeframe.updated !== data.timeframe.published &&
    new Date(data.timeframe.updated);
  ...
  return (
    ...
    >
      <div className="mt-8 flex w-4/5 flex-col items-center max-sm:w-[97.5%]">
                      ...
                      <>
                        {tech.icon?.dark || tech.icon?.light ? (
                          <Image
                            src={
                              tech.icon.dark
                                ? urlFor(tech.icon.dark).url()
                                : urlFor(tech.icon.light).url()
                            }
                            alt="/images/dark.svg"
                            width={16}
                            height={16}
                            className="absolute size-full scale-100 transition-all"
                          />
                        ) : (
                          <>
                            <Image
        ...
        {/* Short description */}
        <p className="text-lg">{data.description.short}</p>
