...
const CopyButton = React.forwardRef<
  ...
  Omit<
    ...
    'children' | 'text'
  > & { className?: string; content: string; text?: string }
...
const SmoothScroll = React.forwardRef<
  HTMLButtonElement,
  ButtonProps & { toId: string }
>(({ toId, ...props }, ref) => {
  return (
    <Button
      ref={ref}
...
const BackToTopButton = React.forwardRef<
  HTMLButtonElement,
  Omit<Omit<ButtonProps, 'size'>, 'variant'>
>(({ className, ...props }, ref) => {
  ...
  return (
    <TooltipWrapper text="Back to top" asChild>
      <Button
        ...
        className={cn('fixed bottom-8 right-8 z-50 transition-all', {
          'opacity-0': !show,
          'opacity-100': show,
        })}
      >
